  ...
  try {
    ...
    if (error || !data) {
      return NextResponse.json(
        { success: false, error: 'Defense not found' },
        { status: 404 }
      )
    }
    ...
    return NextResponse.json(
      { success: false, error: 'Failed to fetch defense' },
      { status: 500 }
    )
  }
...
export const PUT = withAdminAuth(async (request: NextRequest, { user, supabase }) => {
  try {
    ...
    // Get existing defense for audit log
    ...
    console.error('[Admin Defenses] PUT Error:', error)
    return NextResponse.json(
      { success: false, error: 'Failed to update defense' },
      { status: 500 }
    )
  }
}, ADMIN_PERMISSIONS.ENTITIES_UPDATE)

// DELETE - Delete defense
export const DELETE = withAdminAuth(async (request: NextRequest, { user, supabase }) => {
  try {
    ...
    console.error('[Admin Defenses] DELETE Error:', error)
    return NextResponse.json(
      { success: false, error: 'Failed to delete defense' },
      { status: 500 }
    )
  }
